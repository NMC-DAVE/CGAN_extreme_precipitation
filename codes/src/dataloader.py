...
class TiggeMRMSDataset(Dataset):
    """ PyTorch Dataset for TIGGE OBS pairing.

    Returns TiggeOBSDataset object.
    mrms is the obs.
    ...
    def __init__(self, tigge_dir, tigge_vars, mrms_dir, lead_time=12, patch_size=256, rq_fn=None,
                 ...
                 cat_bins=None, pure_sr_ratio=None, dropna=True, ensemble_mode=None, idx_stride=16,
        ...
        self.val_days = val_days  #days for validation
        self.split = split
        ...
        ])  # Merge all TIGGE variables to a dataset
    ####ADD BY JUNXU
        print(self.tigge)
    #####END ADD
        ###changed by Jun Xu: reverse the lat
        self.tigge=self.tigge.reindex(lat=list(reversed(self.tigge.lat)))
        self.tigge['member'] = np.array([0])
        #######change end####

        if 'convective_inhibition' in tigge_vars:
            print("setting nans in convective_inhibition to 0")
            self.tigge['cin'] = self.tigge['cin'].fillna(0)    ###cin nan=0



        self.tigge['tp'] = self.tigge.tp.diff('lead_time')  # Need to take diff to get 3h accumulation and multiply 1000mm
        self.tigge['cp'] = self.tigge.cp.diff('lead_time')  # Need to take diff to get 3h cp accumulation
        self.tigge['lsp'] = self.tigge.lsp.diff('lead_time') # Need to take diff to get 3h lsp accumulation
        self.tigge = self.tigge.sel(lead_time=np.timedelta64(lead_time, 'h'))  ###which lead time to deal 此处已做选择
        self.mrms = xr.open_mfdataset(f'{mrms_dir}/*.nc').tp  # NOTE: Takes around 30s

        ###changed by Jun Xu: reverse the lat and change BJT to UTC
        self.mrms=self.mrms.reindex(lat=list(reversed(self.mrms.lat)))
       # self.mrms.coords['time'] = self.mrms.coords['time']-pd.Timedelta(hours=8) #already UTC
        #######change end####


        # Make sure there are no negative values
        self.tigge['tp'] = self.tigge['tp'].where(self.tigge['tp'] >= 0, 0)
        self.tigge['cp'] = self.tigge['cp'].where(self.tigge['cp'] >= 0, 0)
        self.tigge['lsp'] = self.tigge['lsp'].where(self.tigge['lsp'] >= 0, 0)
        self.mrms = self.mrms.where(self.mrms >= 0.0, 0)
        ...
        self.tigge.load();
        self.mrms.load()  # Load datasets into RAM 载入已选择好预报时效的数据
        if tp_log:
            self.tigge['tp'] = log_trans(self.tigge['tp'], tp_log) ##对降水做log处理
        ...
        ##add by Junxu check max min mrms
        print('this is scale mrms info')
        print(self.mrms.max())
        print(self.mrms.min())
        print('info end')
        ######add end
        self.tigge_km = 8  # ds.tigge.lon.diff('lon').max()*100  # Currently hard-coded CHANGED BY JUN XU FOR ECMWFIFS
        self.mrms_km = 1   #CHANGED BY JUN XU FOR 1KM OBS - SAME RATIO:8:1
...
#####changed by JUN XU:
        self.var_names = {'total_precipitation': 'tp','convective_precipitation': 'cp',
                          'total_column_water': 'tcwv','convective_available_potential_energy': 'cape',
                          'mean_sea_level_pressure': 'msl','geopotential_height_700hpa':'gh',
                          'vwind_850hPa':'v','uwind_850hPa':'u','vwind_925hPa':'vb','uwind_925hPa':'ub',
                          'surface_pressure':'sp','largescalegrid_precipitation':'lsp',
                          'surface_2m_temperature':'st2m',
                          'geopotential_height_500hpa':'hm','uwind_500hPa':'um','vwind_500hPa':'vm'}
#############changed end
    ...
    def _setup_indices(self):
        ...
        ####without radar mask this two sentence
    ...
    def __getitem__(self, idx, time_idx=None, full_array=False, no_cat=False, member_idx=None):
        ...
        **Attention:**
        ...
        if torch.is_tensor(idx): #如果idx是tensor
            idx = idx.tolist() #将idx转化为列表
        ...
        self.var_stack_idxs = {}
        ind_count = 0
       #处理集合预报数据 转置变形 这部分功能ensemble=none 并未开启
        ...
        if self.ensemble_mode == 'stack_by_variable':
            X = xr.concat([X.rename({'variable': 'raw_variable'}).sel(raw_variable=self.var_names[i]).stack(
                variable=['member']).transpose(
                'variable', 'lat', 'lon').drop('raw_variable') for i in self.tigge_vars if 'ens10' in i] +
                          [X.sel(variable=[self.var_names[i] for i in self.tigge_vars if 'ens10' not in i],
                                 member=0).transpose(
                              'variable', 'lat', 'lon')],
                          'variable')
        ...
        if self.pad_tigge_channel:
            X_crop = X[:, self.pad_tigge:self.pad_tigge + self.patch_tigge,
                     self.pad_tigge:self.pad_tigge + self.patch_tigge]
...
class TiggeMRMSHREFDataset(TiggeMRMSDataset):
    def __init__(self, href_dir, **kwargs):
        ...
        ####changed by Jun XU: reverse the lat and expand the memeber dim:
        self.href=self.href.reindex(lat=list(reversed(self.href.lat)))
        self.href = self.href.assign_coords(member='D1').expand_dims('member')
        self.href['member'] = np.array([0])
        #######change end####
        print('prepare to dropna')
        self.href = self.href.dropna('init_time')   ###去除有缺测的时次
        print('dropna complete')

        self.href = self.href.tp.diff('lead_time').sel(lead_time=np.timedelta64(30, 'h')) ###changed here lead time Jun
        ...
        if self.ensemble_mode == 'stack_by_variable':
            ...
            for i, var in enumerate(self.tigge_vars):
                if 'ens10' not in var:
                    self.var_stack_idxs[self.var_names[var]] = ind_count + np.arange(1)
                    ##DEBUG ADD BY JUNXU
                    #print(self.var_stack_idxs[self.var_names[var]])
                    ind_count += 1
...
def create_valid_predictions(model, ds_valid):
    ...
    ###add by Jun Xu to debug the arrary length
    print(preds.shape[1],preds.shape[2])
    print(ds_valid.mrms.lon.isel(lon=slice(0, preds.shape[2])))
    #######end add
    # Convert to xarray
    preds = xr.DataArray(
        ####add by Jun Xu: this could be the stride patch problems
        preds[:,:,:len(ds_valid.mrms.lon.isel(lon=slice(0, preds.shape[2])))],
        ### end add
...
class TiggeMRMSPatchLoadDataset(Dataset):
    ...
    def __init__(self, root_dir, samples_vars={'tp': 1}):
        ...
        self.var_stack_idxs = np.load(self.root_dir + '/configs/var_stack_idxs.npz', allow_pickle=True)[
            'var_stack_idxs']
