...
class LeinDisc(nn.Module):
    def __init__(self, input_channels=1, nonlin = 'leaky_relu'):
        ...
        ###change by Jun pachsize*patchsize*2
    ...
    def forward(self, X, y):
        ...
        #####全场池化####应于patchsize一致 Jun 32->64
        hr = nn.AvgPool2d(32)(hr)
        lr = nn.AvgPool2d(32)(lr)
        #######
...
class Corrector(nn.Module):
    ...
    def forward(self, x):
        x = self.embed(x)
        ###changed by XU JUN 0.1*--> / test the noise 16 to 32
        noise = torch.zeros((x.shape[0], 1,32,32), device=device)
        x = torch.cat((x, noise), axis=1)
        ####
        x = self.process(x)
...
#         self.loss = nn.MSELoss()
        ###changed by jun
        #self.l1loss = gen_ens_mean_L1_weighted()
        ####previous self.l1loss = nn.L1Loss()
        ###end change
...
#            print("nan l1loss")
##changed by Jun Xu to sove the problem

         return gen_ens_mean_L1_weighted(corrected,real) + self.fss_lambda*self.fss(corrected, real, threshold = 0.75)#self.l1loss(corrected,real) + self.fss_lambda * self.fss(corrected, real, threshold=0.5)
...
#            print("nan real")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        forecast_fss_50 = self.fss(condition, real, threshold = 0.75)
        corrected_fss_10 = self.fss(corrected, real, threshold = 0.1)
        corrected_fss_50 = self.fss(corrected, real, threshold = 0.75)
...
#                 print(sample_imgs.shape)
                grid = torchvision.utils.make_grid(sample_imgs)
                self.logger.experiment.add_image('generated_images', grid, self.global_step)
                sample_corrected_lrs = torch.cat([self.upsample_input(F.interpolate(real, scale_factor = 0.125, mode = 'bilinear', align_corners = False, recompute_scale_factor=True)), self.upsample_input(condition[:, 0:1, :, :]), self.upsample_input(sample_corrected_lrs)], dim=0)
...
#             print(self.gen.training)
            ...
            #######add fss & TS
            if "gen_hr_corrected_skill" in self.loss_hparams['gen_loss']:
                loss_gen += self.loss_hparams['gen_loss']["gen_hr_corrected_skill"] * gen_hr_corrected_skill(fake, real)
            if "gen_hr_corrected_ts" in self.loss_hparams['gen_loss']:
                loss_gen += self.loss_hparams['gen_loss']["gen_hr_corrected_ts"] * gen_hr_corrected_ts(fake, real)
            if "gen_lr_corrected_ts" in self.loss_hparams['gen_loss']:
                loss_gen += self.loss_hparams['gen_loss']["gen_lr_corrected_ts"] * gen_lr_corrected_ts(corrected_lr, real)
            if "gen_hr_corrected_pod" in self.loss_hparams['gen_loss']:
                loss_gen += self.loss_hparams['gen_loss']["gen_hr_corrected_pod"] * gen_hr_corrected_pod(fake, real)
            ####end add
            #####add extreme
            if "gen_ens_mean_L1_weighted_extreme" in self.loss_hparams['gen_loss']:
                loss_gen += self.loss_hparams['gen_loss']["gen_ens_mean_L1_weighted_extreme"] * gen_ens_mean_L1_weighted_extreme(fake, real)
            ##end add
                
            ####add by Jun low-resolution weighted ens_mean_L1_weight
            if "ens_mean_lr_L1_weighted" in self.loss_hparams['gen_loss']:
                loss_gen += self.loss_hparams['gen_loss']["ens_mean_lr_L1_weighted"] * gen_ens_lr_mean_L1_weighted(corrected_lr, real)
            ###end add
            self.log('gen_wasser', self.loss_hparams['gen_loss']["wasserstein"]*gen_wasserstein(disc_fake), on_epoch=True, on_step=True, prog_bar=True, logger=True)
            self.log('gen_hrts', self.loss_hparams['gen_loss']["gen_hr_corrected_ts"] * gen_hr_corrected_ts(fake, real), on_epoch=True, on_step=True, prog_bar=True, logger=True)
            self.log('gen_lrts', self.loss_hparams['gen_loss']["gen_lr_corrected_ts"] * gen_lr_corrected_ts(corrected_lr, real), on_epoch=True, on_step=True, prog_bar=True, logger=True)
            self.log('generator_loss', loss_gen, on_epoch=True, on_step=True, prog_bar=True, logger=True)
            return loss_gen 
...
#         print("preds shape", preds.shape)
    ...
    # True negative (TN)
         ...
         for sample in range(x.shape[0]):
            ...
            sample_rmse = xs.rmse(preds.sel(sample=sample).mean('member'), truth.sel(sample=sample), dim=['lat', 'lon']).values
            ###二维矩阵 0 1 判断命中等
            hits_sample, misses_sample, false_alarms_sample, correctnegatives_sample = prep_clf(truth.sel(sample=sample), preds.sel(sample=sample).mean('member'),20)
            
            crps.append(sample_crps)
            rmse.append(sample_rmse)
            ###先计算每个sample的命中数等，然后连接矩阵
            hits.append(np.sum(hits_sample))
            misses.append(np.sum(misses_sample))
            false_alarms.append(np.sum(false_alarms_sample))

        #print(f'this is hitsshape: {len(hits)}') ##输出二维矩阵
         crps = torch.tensor(np.mean(crps), device=self.device)
         rmse = torch.tensor(np.mean(rmse), device=self.device)
        ###先按矩阵计算矩阵分母
         ts_fenmu_sample=np.array(hits)+np.array(misses)+np.array(false_alarms)
        #print('this is ts_fenmu_sample')
        #print(ts_fenmu_sample)
         nonzero_ts_fenmu_sample = ts_fenmu_sample != 0
         hitsa=np.array(hits)
         ts_all = hitsa[nonzero_ts_fenmu_sample]/ts_fenmu_sample[nonzero_ts_fenmu_sample]
        #print(f'this is hitmunonzero {hitsa[nonzero_ts_fenmu_sample].shape}')
        #print(hitsa[nonzero_ts_fenmu_sample])
        #print(f'this is fenmunonzero {ts_fenmu_sample[nonzero_ts_fenmu_sample].shape}')
        #print(ts_fenmu_sample[nonzero_ts_fenmu_sample])
        #print('this is ts_all {ts_all.shape}')
        #print(ts_all) 
         ts=torch.tensor(np.mean(ts_all), device=self.device)
    ...
    # True negative (TN)
         ...
         sample_crps = xs.crps_ensemble(truth, preds).values
         sample_rmse = xs.rmse(preds.mean('member'), truth, dim=['lat', 'lon']).values
            ###二维矩阵 0 1 判断命中等
         hits_sample, misses_sample, false_alarms_sample, correctnegatives_sample = prep_clf(truth, preds.mean('member'),20)
        ...
        #print(f'this is ts {ts}')
        self.log('val_crps', crps, on_epoch=True, on_step=False, prog_bar=True, logger=True, sync_dist=True)
        self.log('val_rmse', rmse, on_epoch=True, on_step=False, prog_bar=True, logger=True, sync_dist=True)
        self.log('ts', ts*100, on_epoch=True, on_step=False, prog_bar=True, logger=True, sync_dist=True)
...
class CorrectorGen2(nn.Module):
    ...
    def forward(self, x, noise):
        x = self.embed(x)
        ###changed by XU JUN 0.1*--> / test the noise
        x = torch.cat((x,0.2*noise), axis=1)
        ####
        x = self.process(x)
        corrected = 1.01*torch.sigmoid(self.corrector(x)) - 0.005
        x = self.upscale(x)
...
###add by Jun XU train generator
class CorrectorGenerator(LightningModule):
    def __init__(self, generator, noise_shape, input_channels=1,
                 cond_idx=0, real_idx=1,
                 gen_spectral_norm=False, zero_noise=True,
                 opt_hparams={'gen_optimiser': 'adam',  'gen_lr': 1e-4,
                              'gen_freq': 1, 'b1': 0.0, 'b2': 0.9},
                 loss_hparams={'gen_loss': "wasserstein", 'lambda_gp': 10},
                 val_hparams={'val_nens': 10, 'tp_log': 0.01, 'ds_max': 50, 'ds_min': 0}):  # fill in
        super().__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % 500 == 0:
            ...
            grid = torchvision.utils.make_grid(sample_imgs)
            self.logger.experiment.add_image('generated_images', grid, self.global_step)
            sample_corrected_lrs = torch.cat([self.upsample_input(
                F.interpolate(real, scale_factor=0.125, mode='bilinear', align_corners=False,
                              recompute_scale_factor=True)), self.upsample_input(condition[:, 0:1, :, :]),
                                              self.upsample_input(sample_corrected_lrs)], dim=0)
            grid = torchvision.utils.make_grid(sample_corrected_lrs)
            self.logger.experiment.add_image('corrected_lr_forecasts', grid, self.global_step)
...
##end add
            ...
            if "ens_mean_lr_L1_weighted" in self.loss_hparams['gen_loss']:
                loss_gen += self.loss_hparams['gen_loss']["ens_mean_lr_L1_weighted"] * gen_ens_lr_mean_L1_weighted(corrected_lr, real)
            ###end add
            self.log('generator_hrts', self.loss_hparams['gen_loss']["gen_hr_corrected_ts"] * gen_hr_corrected_ts(fake, real), on_epoch=True, on_step=True, prog_bar=True, logger=True)
            self.log('generator_lrts', self.loss_hparams['gen_loss']["gen_lr_corrected_ts"] * gen_lr_corrected_ts(corrected_lr, real), on_epoch=True, on_step=True, prog_bar=True, logger=True)
            self.log('generator_loss', loss_gen, on_epoch=True, on_step=True, prog_bar=True, logger=True)
            return loss_gen
    ...
    def configure_optimizers(self):
                ...
                return [{"optimizer": gen_opt, "frequency": self.opt_hparams['gen_freq']}]
    ...
    def validation_step(self, batch, batch_idx):
                ...
                if(len(preds.shape)==4):
                 ...
                 rmse = torch.tensor(np.mean(rmse), device=self.device)
                  ...
                  sample_crps = xs.crps_ensemble(truth, preds).values
                  sample_rmse = xs.rmse(preds.mean('member'), truth,
                                          dim=['lat', 'lon']).values
                  crps.append(sample_crps)
                  rmse.append(sample_rmse)
